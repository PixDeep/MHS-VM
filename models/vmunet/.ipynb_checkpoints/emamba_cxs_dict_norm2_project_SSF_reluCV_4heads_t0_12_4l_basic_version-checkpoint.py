...
class SSFCV_relu(nn.Module):
    ...
    def forward(self, x):
        ...
        # scale = self.act(self.k * x_cv + self.b)
        #t = torch.exp(self.b)
        scale = self.act(x_cv) # *self.s # t=0.5, s=2 x_mean is ok 0.7995
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 9, 2], depths_decoder=[2, 9, 2, 2],
                 dims=[12, 24, 48, 96], dims_decoder=[96, 48, 24, 12], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
