...
class VMMBV0(nn.Module):
    ...
    # scantype: Z
    def x2ys_ZScan(self, x: torch.Tensor):
        ...
        inv_y = torch.flip(ys_mam[:, 2:4], dims=[-1]).view(b, 2, -1, l)
        wh_y = torch.transpose(ys_mam[:, 1].view(b, -1, w, h), dim0=2, dim1=3).contiguous().view(b, -1, l)
        invwh_y = torch.transpose(inv_y[:, 1].view(b, -1, w, h), dim0=2, dim1=3).contiguous().view(b, -1, l)

        # 下面的按 xs 的顺序应该是 xs [0, 2, 1, 3] [原行, 原行翻转, 原竖, 原竖翻转]
        # ys.shape == [b, k, d, l] 
        y1, y2, y3, y4 = ys_mam[:, 0], inv_y[:, 0], wh_y, invwh_y
        ys_repack = torch.stack([y1, y2, y3, y4], dim=1) 
        # 恢复出厂顺序，并返回
        return ys_repack
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.scan_type == 'Z':
            ys = self.x2ys_ZScan(x)
        elif self.scan_type == 'C':
...
class SSFCV_v0(nn.Module):
    def __init__(self, kernel_size=1):
        super().__init__()
        self.conv = nn.Conv2d(5, 1, kernel_size, bias=False)
    ...
    def forward(self, x):
        ...
        x_cv = x_std / (x_new_mean + 0.0001)
        
        #
        x_cat = torch.cat([x_mean, x_min, x_max, x_range, x_std], dim=1)
        x_cov = self.conv(x_cat) * x_cv
        x = x_cov.squeeze(dim=1)

        return x   
    
class SSFCV_sum(nn.Module):
    def __init__(self):
        super().__init__()
        #self.conv = nn.Conv2d(5, 1, kernel_size, bias=False)
...
class SSFCVv1(nn.Module):
    def __init__(self, kernel_size=1):
        ...
        #self.cv_proj = nn.Linear(1, 1, bias=True)
    ...
    def forward(self, x):
        ...
        #
        # x_cv_mean = torch.mean(x_cv, dim=[-2,-1], keepdim=True) # b, 1, 1, 1
        #
        x_cv_posmean = torch.mean(x_cv, dim=[-1], keepdim=True) # b, 1, d, 1
        l = x.shape[3]
        x_cv_posmean = x_cv_posmean.expand(-1,-1,-1,l)
        x_cat = torch.cat([x_mean, x_max, x_cv_posmean], dim=1)
        # scale = self.act(self.k * x_cv + self.b)
        # scale = self.act(self.k * x_cv + self.b)
        #scale = self.act(self.k * x_cv_posmean + self.b)
        x_cov = self.conv(x_cat) #* scale
        # print(torch.min(x_min), torch.max(x_min), scale.shape)
        x = x_cov.squeeze(dim=1)
        # x.shape: b, d, l
        return x       

class SSFCVv3(nn.Module):
    def __init__(self, kernel_size=1):
        super().__init__()
        self.conv = nn.Conv2d(2, 1, kernel_size, bias=False)
        #self.act = nn.Sigmoid() # Sigmoid()
...
class SSFCV_relu(nn.Module):
    def __init__(self):
        super().__init__()
        #self.t = t
        #self.b = nn.Parameter(-2*torch.ones(1))
        #self.t = nn.Parameter(0.1*torch.ones(1))
        #self.conv = nn.Conv2d(3, 1, kernel_size, bias=False)
        self.act = nn.ReLU() 
    ...
    def forward(self, x):
        ...
        #l = x.shape[3]
        #x_cv_posmean = x_cv_posmean.expand(-1,-1,-1,l)
        #x_cat = torch.cat([x_mean, x_max, x_cv], dim=1)
        # scale = self.act(self.k * x_cv + self.b)
        # scale = self.act(self.k * x_cv + self.b)
        #t = torch.exp(self.b)
        scale = self.act(x_cv - 1) # *self.s # t=0.5, s=2 x_mean is ok 0.7995
...
class SSFCV_abs_relu(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x_sum  = torch.sum(x, dim=1, keepdim=True)
        x_std  = torch.std(x, dim=1, keepdim=True)
        #x_abs  = torch.abs(x) #, dim=1, keepdim=True)
        #x_max,_ = torch.max(x, dim=1, keepdim=True)

        #x_abs_mean = torch.mean(x_abs, dim=1, keepdim=True)
        #x_cv = x_std / (x_abs_mean + 1e-5)

        #scale = self.act(x_cv) # 
        x_cov = x_sum * x_std #scale 
        #print('--------', torch.min(x_std), torch.max(x_std), torch.mean(x_std), torch.median(x_std))#, scale.shape)
        #print(torch.min(x_cv), torch.max(x_cv), torch.mean(x_cv), scale.shape)
        x = x_cov.squeeze(dim=1)
        # x.shape: b, d, l
        return x          
...
class PEEM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.ssf_Z = SSFCV_abs_relu( ) 
        self.ssf_C = SSFCV_abs_relu( ) 
        self.ssf_X = SSFCV_abs_relu( ) 
        self.ssf_S = SSFCV_abs_relu( ) 
    ...
    def forward(self, x):
        ...
        b, d, h, w = x.shape
        # 
        x_ZCXS = torch.einsum("b d h w, c d -> b c h w", x, self.x_ssproj_Ws)

        # split to X_C, X_X, X_S
        # x_C, x_X, x_S = rearrange(x_CXS, 'b (k d) h w -> k b d h w', k=n)
        x_Z, x_C, x_X, x_S = torch.split(x_ZCXS, split_size_or_sections=self.d_sub, dim=1)

        # scan in subspaces
        ys_Z = self.mamba_Z(x_Z)
        ...
        y_cat = torch.cat([y_Z, y_C, y_X, y_S], dim=1)
