...
class SSFCV_relu(nn.Module):
    ...
    def forward(self, x):
        ...
        # scale = self.act(self.k * x_cv + self.b)
        #t = torch.exp(self.b)
        scale = self.act(x_cv - 0.8) # *self.s # t=0.5, s=2 x_mean is ok 0.7995
        x_cov = x_sum * scale #self.conv(x_cat) * scale
        # print('-------', torch.min(x_cv), torch.max(x_cv), torch.mean(x_cv), torch.median(x_cv), scale.shape)
        x = x_cov.squeeze(dim=1)
        # x.shape: b, d, l
        return x          
...
class PEEM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.ssf_Z = SSFCV_relu( ) 
        self.ssf_C = SSFCV_relu( ) 
        self.ssf_X = SSFCV_relu( ) 
        self.ssf_S = SSFCV_relu( ) 
