...
class SSFCV_relu(nn.Module):
    ...
    def forward(self, x):
        ...
        return x          
'''    
class SSFCV_avgmax_relu(nn.Module):
    def __init__(self):
        super().__init__()
        #self.t = t
        #self.b = nn.Parameter(-2*torch.ones(1))
    ...
    def forward(self, x):
        ...
        # x.shape: b, k, d, l
        x_mean  = torch.sum(x, dim=1, keepdim=True)
        x_std   = torch.std(x, dim=1, keepdim=True)
        x_min,_ = torch.min(x, dim=1, keepdim=True)
        x_max,_ = torch.max(x, dim=1, keepdim=True)
        # x_range = x_max - x_min
        ...
        scale = self.act(x_cv - 0.5) # *self.s # t=0.5, s=2 x_mean is ok 0.7995
        x_cov = self.conv(x_cat) * scale #self.conv(x_cat) * scale
...
class PEEM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.ssf_Z = SSFCV_avgmax_relu( ) 
        self.ssf_C = SSFCV_avgmax_relu( ) 
        self.ssf_X = SSFCV_avgmax_relu( ) 
        self.ssf_S = SSFCV_avgmax_relu( ) 
